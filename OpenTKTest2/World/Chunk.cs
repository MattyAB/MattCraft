...
namespace OpenTKTest2.World
{
    class Chunk
    {
        ...
        public List<Render.Face> GetRenderFaces()
        {
            List<Render.Face> faces = new List<Render.Face>();
                        ...
                        if(!blockdata[i,j,k].Transparent())
                        {
                            if (i != 0)
                            {
                                if (blockdata[i - 1, j, k].Transparent())
                                    faces.Add(new Render.Face(i, j, k, 0, blockdata[i, j, k].GetTexNo(0)));
                            }
                            else
                                faces.Add(new Render.Face(i, j, k, 0, blockdata[i, j, k].GetTexNo(0)));
                            if (i != 15)
                            {
                                if (blockdata[i + 1, j, k].Transparent())
                                    faces.Add(new Render.Face(i + 1, j, k, 0, blockdata[i, j, k].GetTexNo(1)));
                            }
                            else
                                faces.Add(new Render.Face(i + 1, j, k, 0, blockdata[i, j, k].GetTexNo(1)));
                            ...
                            if (j != 0)
                            {
                                if (blockdata[i, j - 1, k].Transparent())
                                    faces.Add(new Render.Face(i, j, k, 1, blockdata[i, j, k].GetTexNo(2)));
                            }
                            else
                                faces.Add(new Render.Face(i, j, k, 1, blockdata[i, j, k].GetTexNo(2)));
                            if (j != 15)
                            {
                                if (blockdata[i, j + 1, k].Transparent())
                                    faces.Add(new Render.Face(i, j + 1, k, 1, blockdata[i, j, k].GetTexNo(3)));
                            }
                            else
                                faces.Add(new Render.Face(i, j + 1, k, 1, blockdata[i, j, k].GetTexNo(3)));
                            ...
                            if (k != 0)
                            {
                                if (blockdata[i, j, k - 1].Transparent())
                                    faces.Add(new Render.Face(i, j, k, 2, blockdata[i, j, k].GetTexNo(4)));
                            }
                            else
                                faces.Add(new Render.Face(i, j, k, 2, blockdata[i, j, k].GetTexNo(4)));
                            if (k != 15)
                            {
                                if (blockdata[i, j, k + 1].Transparent())
                                    faces.Add(new Render.Face(i, j, k + 1, 2, blockdata[i, j, k].GetTexNo(5)));
                            }
                            else
                                faces.Add(new Render.Face(i, j, k + 1, 2, blockdata[i, j, k].GetTexNo(5)));
                        }
